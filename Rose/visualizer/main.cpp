...
int main(int argc, char *argv[]) {
	...
	// LOSSO MusicPlayer player;
	int frames = FRAMES;
	if (argc > arg) {
		// LOSSO player.load(argv[arg++]);
		// LOSSO frames = (int) (player.length() * framerate);
	}
	...
	while (glfwGetKey(window, GLFW_KEY_ESCAPE) != GLFW_PRESS && !glfwWindowShouldClose(window)) {
		...
		while (!key_queue.empty()) {
			...
			case GLFW_KEY_SPACE:
				...
				if (playing) {
					startframe = frame;
					// LOSSO player.start(frame / (double) framerate);
				} else {
					// LOSSO player.stop();
					frame_set = true;
				}
		...
		if (frame_set) {
			// LOSSO player.set_time(frame / (double) framerate);
		}
		...
		if (playing && project) {
			...
			do {
				usleep(1000);
				// LOSSO frame = (int)(player.get_time() * framerate);
				frame++;
